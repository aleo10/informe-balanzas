...
if b:
    ...
    # Datos de balanza para las secciones siguientes
...
if hacer_ensayo:
    ...
    cfg = get_config()
    st.markdown("**Certificados de pesas** *(tomados de la base de datos — editá el Excel para actualizarlos)*")
    ...
    # ── Simulación de indicadores (error 1 div, prob 5%) ─────────────────────
    # Resetear si cambia la balanza o la carga de ensayo
    ...
    # Decimales según escala de div_min (ej: 20→0, 0.5→1, 0.05→2)
    ...
    # Pre-poblar indicadores de excentricidad
    ...
    if es_camion:
        ...
        with lc2:
            carga_sust  = st.number_input("Carga sustituta (kg)", value=19600, step=20, min_value=20,
                                          help="Peso del camión vacío + autoelevador")
        with lc3:
            pesas_disp  = st.number_input("Pesas disponibles (kg)", value=20000, step=20, min_value=20,
                                          help="Total de pesas propias disponibles (20.000 kg)")
        ...
        carga_max_lin = st.number_input(
            ...
            help="Modificá para agregar o quitar pasos. Los incrementos son por pesa unitaria."
        )
    ...
    # Pre-poblar indicadores de linealidad
...
if _top_generate or _bottom_generate:
    ...
    try:
        ...
        # ── Informe de Ensayo (solo si se hicieron) ───────────────────────────
        if hacer_ensayo:
            if_data = {
                **datos_base,
        ...
        # Actualizar precintos en DB si cambiaron
        if precintos_cambiados:
            update_precintos(
                balanza_sel,
        ...
        # Guardar en session_state para que persistan al hacer clic en descarga
...
if "docs_is_bytes" in st.session_state:
    ...
    with col_d1:
        st.download_button(
            label=f"⬇️ Descargar Informe de Servicio",
            data=st.session_state["docs_is_bytes"],
            file_name=st.session_state["docs_is_name"],
            mime="application/pdf",
            key="dl_is",
    ...
    if st.session_state.get("docs_hacer_ensayo") and st.session_state.get("docs_if_bytes"):
        with col_d2:
            st.download_button(
                label=f"⬇️ Descargar Informe de Ensayo",
                data=st.session_state["docs_if_bytes"],
                file_name=st.session_state["docs_if_name"],
                mime="application/pdf",
                key="dl_if",
...
with st.expander("➕ Agregar Cliente"):
    ...
    if submitted_c:
        ...
        elif new_razon.strip() in get_clientes():
            st.error(f"Ya existe un cliente con ese nombre: {new_razon.strip()}")
        else:
